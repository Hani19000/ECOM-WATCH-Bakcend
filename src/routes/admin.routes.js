...
router.get(
    ...
    asyncHandler(async (_req, res) => {
        ...
        res.status(HTTP_STATUS.OK).json({ status: 'success', data: { jobs } });
    })
);

/**
 * POST /api/v1/admin/crons/:name/execute
 * Déclenche un cron manuellement — réservé aux tests et au debugging.
 */
router.post(
    '/crons/:name/execute',
...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        res.status(HTTP_STATUS.OK).json({ status: 'success', message: `Cron "${name}" arrêté` });
    })
);
...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        res.status(HTTP_STATUS.OK).json({ status: 'success', message: `Cron "${name}" redémarré` });
    })
);
